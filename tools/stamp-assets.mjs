 ...
 * like a deploy that failed. On 2026-09-05 a menu fix was shipped, verified in a
 * real browser, deployed — and still reported as broken, because the browser was
 * being served August's stylesheet. `cf-cache-status: HIT`, `age: 1332`,
 * `last-modified: Mon, 17 Aug 2026`.
 *
 * Changing the URL sidesteps the cache entirely: `main.css?v=ab12cd34` is a
 * different resource, so it is fetched fresh the moment it is deployed. No
 * purge, no API token, nothing to remember.
 *
...
for (const file of htmlFiles(PUBLIC)) {
  ...
  for (const asset of ASSETS) {
    // Matches the asset with or without an existing ?v=, at any depth prefix
    // (writing/ pages link with ../). The prefix is preserved exactly.
    const re = new RegExp(`((?:href|src)=")((?:\\.\\./)*${asset.replace('/', '\\/')})(\\?v=[a-f0-9]+)?(")`, 'g');
